...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  return (
    <div className="space-y-6">
      ...
      {isAdding && (
        <form onSubmit={handleSubmit} className="bg-slate-900 border border-emerald-500/30 p-6 rounded-2xl space-y-4 shadow-2xl animate-in slide-in-from-top-4 duration-300 max-w-lg mx-auto w-full">
          <input 
            value={name}
      ...
      {editingId && (
          <div className="fixed inset-0 bg-slate-950/90 z-50 flex items-center justify-center p-6 backdrop-blur-md">
             <div className="bg-slate-900 border border-slate-800 p-6 rounded-[2rem] w-full max-w-sm space-y-4 shadow-2xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 pb-24">
        {projects.length > 0 ? projects.map(p => (
          ...
          <div className="col-span-full py-24 text-center opacity-20 flex flex-col items-center gap-6 border-2 border-dashed border-slate-800 rounded-[3rem]">
